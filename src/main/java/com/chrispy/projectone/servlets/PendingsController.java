...
public class PendingsController {
	...
	public static void getPendings(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
	
		ReimbursementDao rDao = new ReimbursementDaoImpl();
		
//		User user = (User) req.getSession().getAttribute("currentUser");
		List<Reimbursement> rList = rDao.selectAllReimbursementsByStatus(10);
		
		System.out.println(rList);
		res.setContentType("application/json");
		res.getWriter().write(new ObjectMapper().writeValueAsString(rList));
		System.out.println("Within PendingsController; writing list as JSON");
