...
public class DenialsController {
	
	UserDao uDao = new UserDaoImpl();
	
	public static void getDenials(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
	
		ReimbursementDao rDao = new ReimbursementDaoImpl();
		
//		User user = (User) req.getSession().getAttribute("currentUser");
		List<Reimbursement> rList = rDao.selectAllReimbursementsByStatus(20);
		
		System.out.println(rList);
		res.setContentType("application/json");
		res.getWriter().write(new ObjectMapper().writeValueAsString(rList));
		System.out.println("Within DenialsController; writing list as JSON");
